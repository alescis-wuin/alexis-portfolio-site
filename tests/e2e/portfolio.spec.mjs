...
test('la page d’accueil charge les contenus principaux', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { level: 1 })).toContainText(/Développeur backend/i);
  await expect(page.getByRole('main')).toBeVisible();
  await expect(page.getByRole('link', { name: /Voir les projets/i })).toBeVisible();
  await expect(page.getByRole('link', { name: /Envoyer un e-mail/i })).toBeVisible();
});
...
for (const projectPage of projectPages) {
  test(`la page projet ${projectPage.path} charge correctement`, async ({ page }) => {
    const response = await page.goto(projectPage.path);

    expect(response?.ok()).toBe(true);
    await expect(page).toHaveTitle(projectPage.title);
    await expect(page.getByRole('heading', { level: 1 })).toBeVisible();
    await expect(page.getByRole('link', { name: /Retour aux projets/i })).toBeVisible();
  });
}
